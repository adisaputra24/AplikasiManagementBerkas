...
class AkunController extends Controller
{
    ...
    public function dataTable(Request $request)
    {
        ...
        if (!empty($akun_data)) {
            foreach ($akun_data as $akun_val) {
                $url = secure('akun.edit', ['id' => $akun_val->id]);
                $urlHapus = secure('akun.delete', $akun_val->id);
                if ($akun_val->user_image) {
                    $img = $akun_val->user_image;
                } else {
                    $img = secure_asset('vendor/adminlte3/img/user2-160x160.jpg');
                }
    ...
    public function tambahAkun(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            return redirect()->secure('akun.add')->with('status', 'Data telah tersimpan di database');
        }
    ...
    public function ubahAkun($id, Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            return redirect()->secure('akun.edit', ['id' => $usr->id])->with('status', 'Data telah tersimpan di database');
        }
